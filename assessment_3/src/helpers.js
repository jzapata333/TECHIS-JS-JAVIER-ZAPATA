...
export function shuffle(tiles) {
  // sort tiles by randomizing, then check if its solvable AND if it is not solved already.
  // Otherwise, shuffle tiles again.
  const shuffledTiles = [
    ...tiles
...
export function canSwap(srcIndex, destIndex) {
  // check if the tile you are pressing can move; if the tile pressed is adjacent to the empty slot.
...
export function swap(tiles, src, dest) {
  // function that actually does the swapping. Grabs source tile, and destination tile, and assigns their
